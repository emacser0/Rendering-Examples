...
const char* shadowMapGeometryShaderSource = R"(
#version 330 core
layout (triangles) in;
...
void main()
{
    ...
    if(reverse_normals) // a slight hack to make sure the outer large cube displays lighting from the 'inside'
...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Shadow Cube Mapping", NULL, NULL);
    if (window == NULL) {
    ...
    glUniform1i(glGetUniformLocation(sceneShader, "shadowMap"), 0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // set light uniforms
        glUniform3fv(glGetUniformLocation(sceneShader, "viewPos"), 1, &cameraPos[0]);
        glUniform3fv(glGetUniformLocation(sceneShader, "lightPos"), 1, &lightPos[0]);
        glUniform1f(glGetUniformLocation(sceneShader, "far_plane"), far_plane);
